...
| 保存（缓存） | 💾「保存」（Ctrl/⌘+S）写入缓存：localStorage，文件夹模式再写一份 `*.cache.html` 到源目录；**每 5 分钟自动保存**；保存按钮左侧显示「最后保存：YYYY-MM-DD HH:mm」并实时更新；重开同名文件可一键「↩ 恢复缓存」 |
| 导出（另存为） | ⬇️「导出」选位置写出干净 HTML（不支持文件选择器的浏览器则直接下载）。保存只动缓存、不覆盖源文件，要落地成品用导出 |
